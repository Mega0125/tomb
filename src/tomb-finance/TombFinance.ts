...
/**
 * An API module of TOMP Finance contracts.
...
export class TombFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    this.TOMB = new ERC20(deployments.tomb.address, provider, 'TOMP');
    this.TSHARE = new ERC20(deployments.tShare.address, provider, 'TSHARES');
    this.TBOND = new ERC20(deployments.tBond.address, provider, 'TBOND');
    this.FTM = this.externalTokens['WFTM'];

    // Uniswap V2 Pair
    this.TOMBWFTM_LP = new Contract(externalTokens['TOMP-FTM-LP'][0], IUniswapV2PairABI, provider);
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('TOMP') ? this.TOMB : this.TSHARE;
    const isTomb = name.startsWith('TOMP');
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'TOMP' ? await this.getTombStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    if (depositTokenName.startsWith('TOMP-FTM')) {
      return rewardPerSecond.mul(30000).div(59500);
    } else if (depositTokenName.startsWith('TSHARE-FTM')) {
  ...
  async getDepositTokenPriceInDollars(tokenName: string, token: ERC20) {
    ...
    if (tokenName === 'WFTM') {
      ...
      } else if (tokenName === "TOMP-TSHARE-LP") {
        tokenPrice = await this.getLPTokenPrice(token, this.TOMB, true);
  ...
  async earnedFromBank(
    ...
    try {
      if (earnTokenName === 'TOMP') {
        // problem is pendingTOMP isnt a function since the abi still says pendingTOMB
        return await pool.pendingTOMP(poolId, account);
      } else {
